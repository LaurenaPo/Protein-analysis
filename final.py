...
def common_output(pdb_file, dssp_file, dssp2_file, stride_file,sst_file,bturn_file, gturn_file, bbulge_file):
    ...
    #beta-bulges
    x_seq_bbulge, x_pos_bbulge, first_seq_bbulge, first_pos_bbulge, second_seq_bbulge, second_pos_bbulge, bbluge_type, chain_ID_bbulge = parse_promotif_bbulge(bbulge_file)
    ...
    with open(common_file_parsed, "w+") as file_out:
        ...
        for i in range(len(x_pos_bbulge)):
               Remarque7=Remarque7+"Remark #7 beta-bulges chain : {} pos-x: {} pos1-2 : {}-{} sequence-x : {} sequence1-2 : {}-{} type: {}\n".format(
                    chain_ID_gturn[i],x_pos_bbulge[i],first_pos_bbulge[i],second_pos_bbulge[i],x_seq_bbulge[i],first_seq_bbulge[i],second_seq_bbulge[i],bbluge_type[i])
